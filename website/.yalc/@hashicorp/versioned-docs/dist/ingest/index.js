...
async function ingestDocumentationVersion({ product, version, directory, sha, isLatestVersion, }) {
    ...
    // load
    await Promise.all([
        ...structuredDocuments.map((doc) => db_1.writeDocument(doc)),
        ...structuredNavData.map((doc) => db_1.writeDocument(doc)),
    ]);
    structuredDocuments.forEach((doc) => console.log(doc.fullPath, doc.mdxTransforms));
