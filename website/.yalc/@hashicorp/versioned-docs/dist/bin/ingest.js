...
async function main() {
    ...
    // versions not yet in the metadata list, or with a changed ref, are to be uploaded
    ...
    for (const version of versionsToIngest) {
        ...
        await git_1.checkoutRefAndExecute(version.ref, async () => {
            ...
            // Update metadata object if it exists, otherwise add a new one
            if (metadata) {
    ...
    if (!isDryRun) {
        db_1.writeDocument(versionsMetadata);
    }
}
main();
